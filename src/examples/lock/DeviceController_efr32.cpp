...
xTimerHandle sDeviceTimer; // FreeRTOS app sw timer.
...
void DeviceController::Init()
{
    ...
    // Create FreeRTOS sw timer for Device Controller.
    sDeviceTimer = xTimerCreate("DeviceTmr",      // Just a text name, not used by the RTOS kernel
                                1,                // == default timer period (mS)
                                false,            // no timer reload (==one-shot)
                                (void *)this,     // init timer id = app task obj context
                                (TimerCallbackFunction_t)DeviceTimerEventHandler // timer callback handler
    );
    if (sDeviceTimer == NULL)
    {
        WeaveLogError(Support, "DevCtrl timer create failed");
        return;
    }
...
{    
    if (xTimerIsTimerActive(sDeviceTimer))
    {
        CancelTimer();
    }
    ...
    if (xTimerChangePeriod(sDeviceTimer, aTimeoutMs / portTICK_PERIOD_MS, 100) != pdPASS)
    {
        WeaveLogError(Support, "DevCtrl StartTimer() failed");
        return;
    }
}
...
{
    if (xTimerStop(sDeviceTimer, 0) == pdFAIL)
    {
        WeaveLogError(Support, "DevCtrl CancelTimer() failed");
        return;     
    }
}
