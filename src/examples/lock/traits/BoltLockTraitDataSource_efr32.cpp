                                              ...
                                              const uint64_t & aMustBeVersion, nl::Weave::TLV::TLVReader & aArgumentReader)
{
    ...
    if (aIsMustBeVersionValid)
    {
        ...
        {
            EFR32_LOG("Actual version is 0x%X, while must-be version is: 0x%" PRIx64, GetVersion(), aMustBeVersion);
    ...
    if (aIsExpiryTimeValid)
    {
        ...
        {
            EFR32_LOG("BoltLockChangeRequest Command failed!");
        ...
        {
            EFR32_LOG("BoltLockChangeRequest Command Expired!");
    ...
    EFR32_LOG("BoltLockChangeRequest Command Valid!");

    {
        ...
        while (WEAVE_NO_ERROR == (err = aArgumentReader.Next()))
        {
            ...
            default:
                ...
                EFR32_LOG("Unexpected Tag in CustomCommand");
        ...
        {
            GetDeviceController().PostLockOnCommandRequestEvent(changeRequestParam_Actor, DeviceController::UNLOCK_ACTION);
        }
        else if (changeRequestParam_State == BOLT_STATE_EXTENDED)
        {
            GetDeviceController().PostLockOnCommandRequestEvent(changeRequestParam_Actor, DeviceController::LOCK_ACTION);
        }
        else
        {
    ...
    {
        EFR32_LOG("BoltLockChangeRequest Command Parsed!");
        ...
        EFR32_LOG("Sending Success Response to BoltLockChangeRequest Command");
    ...
    {
        EFR32_LOG("BoltLockChangeRequest Command Error : %d", err);
    }
